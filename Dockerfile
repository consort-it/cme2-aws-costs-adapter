...
ENTRYPOINT ["/bin/sh", "-c"]
CMD ["java -Xmx128m -server -jar /microservice/aws-costs-adapter.jar -Duser.timezone=UTC"]

COPY --from=builder "/microservice/build/libs/aws-costs-adapter-all-1.0.jar" /microservice/aws-costs-adapter.jar
...
@Slf4j
@NoArgsConstructor
public class AwsCostsHttpActionAdapter implements HttpActionAdapter<Object, SparkWebContext> {
    ...
    public Object adapt(int code, SparkWebContext context) {
        ...
        try {
            if (code == 401) {
                Spark.halt(401, objectMapper.writeValueAsString(new AwsCostsAdapterException(Errors.ERR_AUTH_FORBIDDEN, "AwsCostsHttpActionAdapter::adapt")));
            } else if (code == 403) {
                Spark.halt(403, objectMapper.writeValueAsString(new AwsCostsAdapterException(Errors.ERR_AUTH_REQUIRED, "AwsCostsHttpActionAdapter::adapt")));
            } else if (code == 200) {
